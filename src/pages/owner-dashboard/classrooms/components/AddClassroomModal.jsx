...
const AddClassroomModal = ({ isOpen, form, onFormChange, onSave, onClose, isEdit }) => (
  <AnimatePresence>
    {isOpen && (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 animate-fadeIn"
          onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-gray-900">{isEdit ? "Edit Classroom" : "Add Classroom"}</h2>
              <p className="text-sm text-gray-500 mt-0.5">{isEdit ? "Modify classroom mapping & financial parameters" : "Create a new classroom or program mapping with Procare IDs"}</p>
            </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={form.procareClassroomId} onChange={(e) => onFormChange("procareClassroomId", e.target.value)}
                  placeholder="e.g. 292" className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F] font-mono" />
              </div>
              <div>
            ...
            <button onClick={onSave}
              className="w-full py-3 rounded-xl bg-[#1E3A5F] text-white text-sm font-semibold hover:bg-[#15294A] transition-colors mt-2">
              {isEdit ? "Save Changes" : <><Plus size={16} className="inline mr-1.5" /> Add Classroom</>}
            </button>
          </div>
        </motion.div>
      </div>
