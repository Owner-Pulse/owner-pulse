...
const initialCompliance = [
  ...
  },
  {
    ...
    expires: "2026-10-01",
    category: "insurance",
    shopReminder: "2026-08-02",
  },
  {
    id: 11,
    item: "Auto / Vehicle Insurance",
    authority: "Progressive",
    status: "compliant",
    expires: "2027-02-14",
    category: "insurance",
    shopReminder: "2026-12-16",
  },
  // Contracts — major vendor agreements with renewal dates
  {
    id: 12,
    item: "Building Lease",
    authority: "Lakeside Properties LLC",
    status: "compliant",
    expires: "2028-08-01",
    category: "contract",
  },
  {
    id: 13,
    item: "Food Service Contract",
    authority: "Sunshine Catering",
    status: "expiring",
    expires: "2026-07-31",
    category: "contract",
  },
...
const initialTasks = [
  ...
  {
    id: 2,
    title: "Renew faculty CPR certifications",
    assignee: "director",
    priority: "high",
    status: "open",
    due: "2026-05-20",
  ...
  {
    id: 4,
    title: "Submit Step Up reconciliation",
    assignee: "director",
    priority: "high",
    status: "open",
    due: "2026-05-30",
  ...
  {
    id: 5,
    title: "Spring carnival vendor confirmation",
    assignee: "director",
    priority: "low",
    status: "in_progress",
    due: "2026-06-02",
  ...
  {
    id: 6,
    title: "Final report card review",
    assignee: "director",
    priority: "medium",
    status: "open",
    due: "2026-06-05",
  ...
  {
    id: 7,
    title: "HVAC vendor decision sign-off",
    assignee: "owner",
    priority: "high",
    status: "open",
    due: "2026-05-12",
  ...
  {
    id: 8,
    title: "Approve PO — Playground equipment",
    assignee: "owner",
    priority: "high",
    status: "open",
    due: "2026-05-14",
  ...
  {
    id: 9,
    title: "Review 5th grade teacher performance",
    assignee: "owner",
    priority: "medium",
    status: "in_progress",
    due: "2026-05-22",
  ...
  {
    id: 10,
    title: "Sign Step Up Q4 attestation",
    assignee: "owner",
    priority: "medium",
    status: "open",
    due: "2026-05-28",
...
const initialDirectorExpenses = [
  {
    id: 1,
    amount: 47.5,
    description: "Pizza for parent meeting",
    date: "2026-04-12",
  },
  {
    id: 2,
    amount: 124.0,
    description: "Crayons and markers — PreK3",
    date: "2026-04-15",
  },
  {
    id: 3,
    amount: 38.0,
    description: "Coffee and donuts for staff PD",
    date: "2026-04-22",
  },
  {
    id: 4,
    amount: 89.0,
    description: "Cleaning wipes restock",
    date: "2026-04-28",
  },
  {
    id: 5,
    amount: 215.0,
    description: "Construction paper bulk order",
    date: "2026-05-01",
  },
  {
    id: 6,
    amount: 65.0,
    description: "Birthday cake for office party",
    date: "2026-05-04",
  },
  {
    id: 7,
  ...
  },
  {
    id: 8,
    amount: 42.0,
    description: "Gift cards for teacher appreciation",
    date: "2026-05-08",
  },
];
...
const Pill = ({ children, tone = "neutral", size = "sm" }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 ${padding} rounded-full font-semibold uppercase tracking-wide`}
  ...
  <div className="flex items-end justify-between mb-4">
    <div>
      <div className="flex items-center gap-2 mb-0.5">
        ...
        <h2 className="text-xl md:text-2xl font-bold" style={{ color: C.ink }}>
          {title}
        </h2>
      </div>
      {sub && (
...
const DonutKPI = ({
  ...
  <Card accent={accentColor} className="p-5">
    ...
    <div className="mt-3 text-center">
      <div className="text-sm font-semibold" style={{ color: C.ink2 }}>
        {big}
      </div>
      {sub && (
...
const OwnerOverview = ({
  ...
  const preEnrolled = classrooms
    .filter((c) => c.tier === "preschool")
    .reduce((a, c) => a + c.enrolled, 0);
  const k8Enrolled = classrooms
    .filter((c) => c.tier === "k8")
    .reduce((a, c) => a + c.enrolled, 0);
  const totalOpenSpots = totalCap - totalEnrolled;

  // Withdrawals YTD: preschool tracked + at-risk who left
  const preWithdrawals = classrooms
    .filter((c) => c.tier === "preschool")
    .reduce((a, c) => a + (c.withdrawals || 0), 0);
  ...
  const coverageMessage = subToday
    ? todaysSubs.length === 1
      ? "1 sub in today"
      : `${todaysSubs.length} subs in today`
    : "No subs today";
  const coverageTone = subToday ? "amber" : "green";

  const directorSpent = directorExpenses.reduce((a, e) => a + e.amount, 0);
  const totalSpent = SCHOOL_BUDGET_CATEGORIES.reduce(
    (a, c) =>
      a + (c.name === "Director's Discretionary" ? directorSpent : c.spent),
    0,
  );
  ...
  const myOpenTasks = tasks.filter(
    (t) => t.assignee === "owner" && t.status !== "done",
  ).length;
  const myHighTasks = tasks.filter(
    (t) =>
      t.assignee === "owner" && t.priority === "high" && t.status !== "done",
  ).length;
  const dirOpenTasks = tasks.filter(
    (t) => t.assignee === "director" && t.status !== "done",
  ).length;

  // Compliance
  const complianceExpired = compliance.filter(
    (c) => c.status === "expired",
  ).length;
  const complianceExpiring = compliance.filter(
    (c) => c.status === "expiring",
  ).length;
  const nextDeadline =
    compliance
  ...
  const discountMonthlyValue = discounts.reduce(
    (a, d) => a + d.monthlyValue,
    0,
  );
  ...
  const turnaroundPct = stepUpApproved.length
    ? Math.round((approvedFast / stepUpApproved.length) * 100)
    : 100;
  const oldestPending = Math.max(
    0,
    ...stepUp
      .filter((s) => s.status === "pending")
      .map((s) => daysSince(s.sent)),
  );
  const stepUpRedFlags = stepUp.filter(
    (s) => s.status === "pending" && daysSince(s.sent) >= 20,
  );
  ...
  const weekExpenses = directorExpenses.filter(
    (e) => new Date(e.date) >= sevenDaysAgo,
  );
  const lastEntry = [
    ...log,
    ...directorExpenses.map((e) => ({ ...e, type: "expense" })),
  ].sort((a, b) => b.date.localeCompare(a.date))[0];
  const lastDate = lastEntry ? lastEntry.date : null;
  const hoursSinceLastEntry = lastDate
    ? Math.round((TODAY - new Date(lastDate)) / 3600000)
    : null;
  ...
  const criticalCompliance = compliance.filter(
    (c) =>
      c.status === "expired" ||
      (c.status === "expiring" && daysUntil(c.expires) <= 14),
  );
  ...
  const pendingDecisions = [
    {
      title: "HVAC Replacement Decision",
      detail:
        "3 quotes received. CoolAir $8,200 · AirPro $8,800 · QuickCool $9,400",
      due: "2026-05-12",
    },
    {
      title: "Approve Playground Equipment",
      detail: "Vendor selected. PO needed for $3,200.",
      due: "2026-05-14",
    },
    {
      title: "Roof Repair — Room 3B",
      detail: "DCF flagged. Leak active. Contractor standing by.",
      due: "2026-05-08",
    },
    {
      title: "Tablet Purchase K–2",
      detail: "12 tablets. Budget pre-approved. Need PO sign-off.",
      due: "2026-05-16",
    },
  ];
  ...
  const inputItemCount =
    ...
    myTasks.length +
    criticalCompliance.length +
    criticalMaintenance.length +
    staleRisk.length +
    stepUpRedFlags.length +
    (payrollDays <= 3 ? 1 : 0);
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <Pill tone={inputItemCount > 0 ? "red" : "green"}>
              {inputItemCount} {inputItemCount === 1 ? "item" : "items"}
            </Pill>
          </div>
          <button
        ...
        {inputItemCount === 0 && (
          <Card className="p-8 text-center" accent={C.green}>
            ...
            <div className="text-sm mt-2" style={{ color: C.ink3 }}>
              Director has the rest covered. Go enjoy your day.
            </div>
          </Card>
        )}
          ...
          <Card className="p-4 flex items-center gap-3" accent={C.red}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="font-bold text-sm" style={{ color: C.ink }}>
                {new Date(NEXT_PAYROLL).toLocaleDateString("en-US", {
                  weekday: "long",
                  month: "long",
                  day: "numeric",
                })}
        ...
        {pendingDecisions.length > 0 && (
          <Card className="p-5" accent={C.amber}>
            <SectionHead
              ...
              sub={`${pendingDecisions.length} pending`}
            />
            <div className="space-y-3">
              {pendingDecisions.map((d, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="p-4 rounded-xl flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2"
        ...
        {myTasks.length > 0 && (
          <Card className="p-5" accent={C.blue}>
            <SectionHead
              icon="📋"
              title="My Tasks"
              sub={`${myTasks.length} open · Owner-assigned`}
              action={
                <Btn kind="ghost" onClick={() => setTab("tasks")}>
                  Open Tasks →
                </Btn>
              }
            />
        ...
        {criticalCompliance.length > 0 && (
          <Card className="p-5" accent={C.red}>
            <SectionHead
              ...
              sub={`${criticalCompliance.length} expired or expiring within 14 days`}
              action={
                <Btn kind="ghost" onClick={() => setTab("compliance")}>
                  Open Compliance →
                </Btn>
              }
            />
            <div className="space-y-2">
              {criticalCompliance.map((c) => (
                ...
                >
                  ...
                  <Pill tone={c.status === "expired" ? "red" : "amber"}>
                    {c.status}
                  </Pill>
                </div>
              ))}
        ...
        {stepUpRedFlags.length > 0 && (
          <Card className="p-5" accent={C.red}>
            <SectionHead
              icon="🎓"
              title="Step Up Stuck"
              sub={`${stepUpRedFlags.length} pending 20+ days · need owner-level escalation`}
              action={
                <Btn kind="ghost" onClick={() => setTab("scholarships")}>
                  Open Scholarships →
                </Btn>
              }
            />
              ...
              {stepUpRedFlags.map((s) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-xs mt-0.5" style={{ color: C.ink3 }}>
                      {fmtMoney(s.amount)} · last contact {s.lastContact}
                    </div>
                  </div>
        ...
        {staleRisk.length > 0 && (
          <Card className="p-5" accent={C.red}>
            <SectionHead
              icon="⚠️"
              title="At-Risk Cases Going Cold"
              sub={`${staleRisk.length} not touched in over 7 days`}
              action={
                <Btn kind="ghost" onClick={() => setTab("enrollment")}>
                  Open Enrollment →
                </Btn>
              }
            />
        ...
        {criticalMaintenance.length > 0 && (
          <Card className="p-5" accent={C.red}>
            <SectionHead
              ...
              sub={`${criticalMaintenance.length} critical-priority requests`}
              action={
                <Btn kind="ghost" onClick={() => setTab("maintenance")}>
                  Open Maintenance →
                </Btn>
              }
            />
              ...
              {criticalMaintenance.map((m) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-xs mt-0.5" style={{ color: C.ink3 }}>
                      {m.location} · est. {fmtMoney(m.estCost)}
                    </div>
                  </div>
    ...
    <div className="space-y-6">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xl md:text-2xl font-extrabold mt-1">
              Payroll Ready — Submit in ADP
            </div>
            <div className="text-sm mt-1" style={{ color: "#FED7AA" }}>
              Director submitted for pay period ending{" "}
              {payrollReady.periodEnding} · {payrollReady.pto?.length || 0} PTO
              · {payrollReady.hoursToAdd?.length || 0} hour adjustments ·{" "}
              {payrollReady.childCare?.length || 0} deductions
            </div>
          </div>
          <button
            onClick={dismissPayrollAlert}
            className="px-5 py-3 rounded-xl text-sm font-bold whitespace-nowrap"
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          style={{
            background: inputItemCount > 0 ? C.red : C.card,
            color: inputItemCount > 0 ? "#FFF" : C.ink,
            border: `1px solid ${inputItemCount > 0 ? C.red : C.line}`,
          }}
        >
          Needs my input{" "}
          {inputItemCount > 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Card
          ...
          accent={
            payrollDays <= 3 ? C.red : payrollDays <= 7 ? C.amber : C.blue
          }
        >
          <div
          ...
          <Pill
            tone={
              payrollDays <= 3 ? "red" : payrollDays <= 7 ? "amber" : "blue"
            }
          >
            {payrollDays}d
          </Pill>
        ...
        <Card
          className="p-4 flex items-center gap-3"
          accent={
            hoursSinceLastEntry !== null && hoursSinceLastEntry < 24
              ? C.green
              : C.amber
          }
        >
            ...
            >
              {hoursSinceLastEntry === null
                ...
                : hoursSinceLastEntry < 24
                  ? `Active · last log ${hoursSinceLastEntry}h ago`
        ...
        <Card
          className="p-4 flex items-center gap-3"
          accent={subToday ? C.amber : C.green}
        >
          <div
            className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
            style={{ background: subToday ? C.amberSoft : C.greenSoft }}
          >
          ...
          <Pill tone={subToday ? "amber" : "green"}>
            {subToday ? "Sub" : "No sub"}
          </Pill>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        ...
        <DonutKPI
          ...
          label="Budget"
          percent={budgetPct}
          big={`${fmtMoneyShort(totalSpent)} / ${fmtMoneyShort(SCHOOL_BUDGET_TOTAL)}`}
          sub="Tap to view →"
        ...
        <NumberKPI
          emoji="✅"
          label="Compliance"
          value={Math.max(nextDeadline, 0)}
          sub="days to deadline"
          accentColor={C.red}
          urgentText={
            complianceExpired + complianceExpiring > 0
              ? `${complianceExpired + complianceExpiring} urgent`
              : null
          }
        />
        <DonutKPI
          emoji="🎓"
          label="Step Up Turnaround"
          percent={turnaroundPct}
          big={`${stepUpPending} pending`}
          sub={
            oldestPending >= 20
              ? `oldest ${oldestPending}d ⚠️`
              : `oldest ${oldestPending}d`
          }
          accentColor={oldestPending >= 20 ? C.red : C.amber}
        />
        <NumberKPI
          emoji="⚠️"
          label="At Risk"
          ...
          sub={
            staleRisk.length > 0
              ? `${staleRisk.length} stale 🚩`
              : "intervening"
          }
          accentColor={C.red}
          urgentText={staleRisk.length > 0 ? "Action needed" : null}
        />
        <NumberKPI
          emoji="📝"
          label="Waitlist"
          value={waitlist.length}
          sub={`${waitlist.filter((w) => PRESCHOOL_PROGRAMS.includes(w.program)).length} pre · ${waitlist.filter((w) => K8_PROGRAMS.includes(w.program)).length} K-8`}
        ...
        <NumberKPI
          ...
          sub={`${discountPct}% · ${fmtMoneyShort(discountMonthlyValue)}/mo`}
          accentColor="#0EA5E9"
        />
        <NumberKPI
          emoji="📋"
          label="My Tasks"
          value={myOpenTasks}
          sub={`${dirOpenTasks} on Director`}
          accentColor={C.green}
          urgentText={myHighTasks > 0 ? `${myHighTasks} high` : null}
        />
      </div>

      {/* DIGESTS — Snapshot intel */}
        ...
        <Card className="p-5" accent={C.purple}>
          <div className="flex items-start justify-between mb-4 gap-2">
            <div>
              ...
              <p className="text-sm mt-0.5" style={{ color: C.ink3 }}>
                Snapshot of what matters · no ops detail
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="p-3 rounded-xl" style={{ background: "#F9FAFC" }}>
              ...
              >
                {fmtMoneyShort(weekExpenses.reduce((a, e) => a + e.amount, 0))}
              </div>
              <div
            ...
            <div className="p-3 rounded-xl" style={{ background: "#F9FAFC" }}>
              ...
              <div className="text-xs mt-0.5" style={{ color: C.ink2 }}>
                {preEnrolled} pre · {k8Enrolled} K-8
              </div>
            </div>
            ...
            <div className="p-3 rounded-xl" style={{ background: "#F9FAFC" }}>
              <div
                className="text-2xl font-extrabold"
                style={{ color: totalWithdrawals > 5 ? C.red : C.amber }}
              >
                {totalWithdrawals}
              </div>
              <div
            ...
            <div className="p-3 rounded-xl" style={{ background: "#F9FAFC" }}>
              <div
                className="text-2xl font-extrabold"
                style={{ color: totalOpenSpots > 0 ? C.green : C.red }}
              >
                {totalOpenSpots}
              </div>
              <div
          ...
          <div className="mt-3 text-xs" style={{ color: C.ink2 }}>
            {dirOpenTasks > 0 ? (
              <>📋 {dirOpenTasks} open tasks on her plate.</>
            ) : (
              <>📋 No open tasks on Director.</>
            )}{" "}
            {oldestPending >= 20 ? (
              <span style={{ color: C.red, fontWeight: 600 }}>
                ⚠ {oldestPending}d-old Step Up approval needs attention.
              </span>
            ) : (
              <>Step Up collections on track.</>
            )}
          </div>
        </Card>

        {/* OWNER DAILY BRIEF — Glancable intel feed */}
      </div>
          ...
          {events.map((e, i) => {
            ...
            return (
              <div key={i} className="py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-xs md:text-sm" style={{ color: C.ink3 }}>
...
const OwnerEnrollment = ({ classrooms, atRisk, setAtRisk, discounts }) => {
  ...
  const discountsByType = ["staff_child", "sibling", "hardship", "other"]
    .map((t) => ({
      type: t.replace("_", " "),
      count: discounts.filter((d) => d.type === t).length,
      value: discounts
        .filter((d) => d.type === t)
        .reduce((a, d) => a + d.monthlyValue, 0),
    }))
    .filter((g) => g.count > 0);
  ...
  const updateRiskStatus = (id, status) => {
    ...
    setAtRisk(
      atRisk.map((x) =>
        x.id === id ? { ...x, status, lastUpdated: today } : x,
      ),
    );
  };
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="👶"
        title="Enrollment"
        sub={`${total} students across ${classrooms.length} classrooms`}
      />
      ...
      <Card className="p-5" accent={C.blue}>
        <div className="flex items-start justify-between mb-4 gap-2">
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: C.ink3 }}>
              Set each year · this dashboard tracks progress
            </p>
          </div>
        </div>
          ...
          {[
            {
              ...
              actual: total,
              target: ENROLLMENT_TARGETS.total,
              color: C.blue,
            },
            {
              label: "Preschool",
              actual: preEnrolled,
              target: ENROLLMENT_TARGETS.preschool,
              color: C.purple,
            },
            {
              label: "K–8",
              actual: k8Enrolled,
              target: ENROLLMENT_TARGETS.k8,
              color: C.orange,
            },
          ].map((t) => {
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Pill
                    tone={met ? "green" : close ? "amber" : "red"}
                    size="xs"
                  >
                    {status}
                  </Pill>
      ...
      <Card className="p-5" accent={activeRisk.length > 0 ? C.red : C.green}>
        <div className="flex items-start justify-between gap-2 mb-4">
          ...
          <Pill tone={activeRisk.length > 0 ? "red" : "green"}>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="p-2 rounded-lg" style={{ background: C.redSoft }}>
            <div className="text-xl font-extrabold" style={{ color: C.red }}>
              {activeRisk.length}
            </div>
            <div
          ...
          <div className="p-2 rounded-lg" style={{ background: "#F9FAFC" }}>
            <div className="text-xl font-extrabold" style={{ color: C.ink3 }}>
              {lostRisk}
            </div>
            <div
              className="text-[10px] font-bold uppercase tracking-wider mt-0.5"
        ...
        {staleRisk.length > 0 && (
          <div
            className="mb-3 p-3 rounded-xl flex items-center gap-2"
            style={{ background: C.redSoft, border: `1px solid ${C.red}` }}
          >
            <AlertTriangle size={16} color={C.red} />
            <div className="text-xs" style={{ color: C.ink }}>
              <span className="font-bold">{staleRisk.length} stale</span> ·{" "}
              {staleRisk.length === 1 ? "this case has" : "these cases have"}{" "}
              not been touched in over 7 days. Status update needed.
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs mt-0.5" style={{ color: C.ink3 }}>
                      Flagged {r.flagged} · {days}d ago · last touched{" "}
                ...
                <div className="flex gap-1.5 mt-2">
                  {["intervening", "retained", "lost"].map((s) => (
                    <button
                      ...
                      style={{
                        background: r.status === s ? C.red : "#EEF1F6",
                        color: r.status === s ? "#FFF" : C.ink2,
                      }}
                    >
                      {s}
                    </button>
          ...
          {activeRisk.length === 0 && (
            <div className="py-6 text-center text-sm" style={{ color: C.ink3 }}>
      ...
      <Card className="p-5" accent="#0EA5E9">
        <div className="flex items-start justify-between gap-2 mb-4">
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: C.ink3 }}>
              Who's getting a break and how much it costs
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="p-3 rounded-xl" style={{ background: "#F9FAFC" }}>
            ...
            <div className="text-[10px]" style={{ color: C.ink3 }}>
              {discountPct}% of school
            </div>
          </div>
          <div className="p-3 rounded-xl" style={{ background: "#F9FAFC" }}>
            <div
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ color: C.ink3 }}>
                <th className="text-left py-2 text-xs font-bold uppercase tracking-wider">
                  Program
                </th>
                <th className="text-right py-2 text-xs font-bold uppercase tracking-wider">
                  Enrolled
                </th>
                <th className="text-right py-2 text-xs font-bold uppercase tracking-wider">
                  Capacity
            ...
            <tbody>
              {byProgram.map((p) => (
                ...
                >
                  <td className="py-3 font-semibold" style={{ color: C.ink }}>
                    {p.program}
                  </td>
                  <td
                  ...
                  <td className="py-3 text-right" style={{ color: C.ink2 }}>
                    {p.capacity}
                  </td>
                  <td
                    className="py-3 text-right font-semibold"
                    style={{
                      color: p.capacity - p.enrolled === 0 ? C.red : C.green,
                    }}
                  >
...
const OwnerClassrooms = ({ classrooms }) => {
  ...
  const totalProfit = list.reduce(
    (a, c) => a + classroomEconomics(c).monthlyProfit,
    0,
  );
  const profitable = list.filter(
    (c) => classroomEconomics(c).monthlyProfit > 0,
  ).length;
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="🏫"
        title="Classrooms — P&L + Performance"
        sub="Is this class profitable? Is the teacher keeping kids and delivering results?"
      />
      ...
      <div className="flex gap-2">
        {[
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl text-xs font-bold transition-all"
            style={{
              background: tierFilter === f.id ? C.blue : C.card,
              color: tierFilter === f.id ? "#FFF" : C.ink,
              border: `1px solid ${tierFilter === f.id ? C.blue : C.line}`,
            }}
          >
            {f.label}
          </button>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="p-4" accent={totalProfit > 0 ? C.green : C.red}>
          ...
          <div
            ...
            style={{ color: totalProfit > 0 ? C.green : C.red }}
          >
            {totalProfit < 0 ? "-" : ""}
            {fmtMoneyShort(Math.abs(totalProfit))}
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {list.map((c) => {
          ...
          const nweaDelta =
            c.nweaMap && nweaBenchmark ? c.nweaMap - nweaBenchmark : null;
          const nweaTone =
            nweaDelta === null
              ? "neutral"
              : nweaDelta >= 5
          ...
          return (
            <Card
              key={c.id}
              className="p-5"
              accent={profitable ? C.green : C.red}
            >
              {/* Header */}
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <Pill tone={profitable ? "green" : "red"} size="xs">
              ...
              <div
                ...
                style={{ background: profitable ? C.greenSoft : C.redSoft }}
              >
                <div className="flex items-baseline justify-between">
                  <div>
                        ...
                        return (
                          d && (
                            ...
                            >
                              ...
                              {fmtMoneyShort(
                                Math.abs(
                                  econ.monthlyProfit - c.lastYear.monthlyProfit,
                                ),
                              )}{" "}
                              vs LY
                            </span>
                    ...
                    >
                      ...
                      {fmtMoney(Math.abs(econ.monthlyProfit))}
                    </div>
                  </div>
                  <div className="text-right">
                    <div
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      color:
                        performanceSignal.tone === "green"
                          ? C.green
                          : performanceSignal.tone === "amber"
                            ? C.amber
                            : C.red,
                    }}
                  >
                  ...
                  >
                    <span>
                      {c.tier === "k8" ? "this room" : "kids who left"}
                    </span>
                    {(() => {
                      const ly =
                        c.tier === "k8"
                          ? c.lastYear?.incidents
                          : c.lastYear?.withdrawals;
              ...
              <div
                ...
                style={{ borderTop: `1px solid ${C.line}`, color: C.ink2 }}
              >
                {!profitable && (
                  <span style={{ color: C.red, fontWeight: 600 }}>
                    ⚠ Losing money.{" "}
                ...
                {c.tier === "k8" && c.incidents > 2 && (
                  <span style={{ color: C.red, fontWeight: 600 }}>
                    ⚠ High incident count.{" "}
                  </span>
                )}
                {c.tier === "preschool" && c.withdrawals > 1 && (
                  <span style={{ color: C.red, fontWeight: 600 }}>
                    ⚠ Losing kids.{" "}
                  </span>
                )}
                {c.tier === "k8" && nweaDelta !== null && nweaDelta < -3 && (
                  <span style={{ color: C.red, fontWeight: 600 }}>
                    ⚠ Below benchmark.{" "}
                  </span>
                )}
                {profitable &&
                  (c.tier === "k8"
                    ? c.incidents <= 2 && (nweaDelta === null || nweaDelta >= 0)
                    : c.withdrawals === 0) && (
...
const OwnerCompliance = ({ compliance }) => {
  ...
  const regulatory = compliance.filter(
    (c) => c.category === "regulatory" || !c.category,
  );
    ...
    <div key={c.id} className="py-3 flex items-center justify-between gap-2">
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs mt-0.5" style={{ color: C.ink3 }}>
          ...
          {showShop &&
            c.shopReminder &&
            daysUntil(c.shopReminder) <= 60 &&
            daysUntil(c.expires) > 0 && (
              <span
                className="ml-2"
                style={{ color: C.amber, fontWeight: 600 }}
              >
                · Shop rates by {c.shopReminder}
              </span>
            )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        ...
        sub="Regulatory, insurance & vendor agreements"
      />
      ...
      <div className="grid md:grid-cols-3 gap-3">
        ...
        <Card className="p-5" accent={counts.expired ? C.red : C.green}>
          ...
          <div
            className="text-5xl font-extrabold mt-2"
            style={{ color: counts.expired ? C.red : C.green }}
          >
            {counts.expired}
          </div>
          <div className="text-xs mt-1" style={{ color: C.ink3 }}>
            {counts.expired ? "Action required" : "All good"}
          </div>
        </Card>
      </div>
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm font-bold mt-1" style={{ color: C.ink }}>
                {insuranceShopAlerts.length} insurance{" "}
                {insuranceShopAlerts.length === 1
                  ? "policy renews"
                  : "policies renew"}{" "}
                soon
              </div>
      ...
      <Card className="p-5" accent={C.blue}>
        <div className="flex items-baseline justify-between mb-3">
          ...
          <div className="text-[10px]" style={{ color: C.ink3 }}>
            Shop rates 60 days before renewal
          </div>
        </div>
        <div className="divide-y" style={{ borderColor: C.line }}>
...
const OwnerTasks = ({ tasks }) => {
  ...
  const list =
    filter === "all" ? tasks : tasks.filter((t) => t.assignee === filter);
  const open = list.filter((t) => t.status !== "done");
  const high = open.filter((t) => t.priority === "high");

  const ownerOpen = tasks.filter(
    (t) => t.assignee === "owner" && t.status !== "done",
  ).length;
  const dirOpen = tasks.filter(
    (t) => t.assignee === "director" && t.status !== "done",
  ).length;
  ...
  const splitData = [
    {
      name: "Owner Open",
      value: tasks.filter((t) => t.assignee === "owner" && t.status === "open")
        .length,
      color: C.red,
    },
    {
      name: "Owner WIP",
      value: tasks.filter(
        (t) => t.assignee === "owner" && t.status === "in_progress",
      ).length,
      color: C.amber,
    },
    {
      name: "Owner Done",
      value: tasks.filter((t) => t.assignee === "owner" && t.status === "done")
        .length,
      color: C.green,
    },
    {
      name: "Director Open",
      value: tasks.filter(
        (t) => t.assignee === "director" && t.status === "open",
      ).length,
      color: C.red,
    },
    {
      name: "Director WIP",
      value: tasks.filter(
        (t) => t.assignee === "director" && t.status === "in_progress",
      ).length,
      color: C.amber,
    },
    {
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="📋"
        title="Tasks"
        sub={`${ownerOpen} on you · ${dirOpen} on Director · ${high.length} high priority`}
      />
      ...
      <div className="flex gap-2">
        {[
          { id: "all", label: "All" },
          { id: "owner", label: "On You" },
          { id: "director", label: "On Director" },
        ].map((f) => (
          <button
            key={f.id}
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                >
                  {t.status === "done" ? (
                    ...
                    <Clock
                      size={16}
                      color={t.status === "in_progress" ? C.amber : C.red}
                    />
                  )}
                </div>
                <div className="min-w-0">
                  ...
                  >
                    <Pill
                      tone={t.assignee === "owner" ? "blue" : "purple"}
                      size="xs"
                    >
                      {t.assignee}
                    </Pill>
...
const OwnerMaintenance = ({ maintenance }) => {
  ...
  const byPriority = ["critical", "high", "medium", "low"].map((p) => ({
    priority: p,
    count: maintenance.filter((m) => m.priority === p && m.status !== "done")
      .length,
    cost: maintenance
      .filter((m) => m.priority === p && m.status !== "done")
      .reduce((a, m) => a + m.estCost, 0),
  }));
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="🔧"
        title="Maintenance"
        sub={`${open.length} open requests · ${fmtMoney(open.reduce((a, m) => a + m.estCost, 0))} estimated`}
      />
        ...
        <div className="space-y-2">
          {maintenance.map((m) => (
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <Pill tone={PRIORITY_TONE[m.priority]} size="xs">
                    {m.priority}
                  </Pill>
                ...
                <div className="text-xs mt-1" style={{ color: C.ink3 }}>
                  {m.location} · logged {m.logged} · est. {fmtMoney(m.estCost)}
                </div>
              </div>
              <Pill
                tone={
...
const OwnerBudget = ({ directorExpenses }) => {
    ...
    if (selectedCategory === "Director's Discretionary") {
      ...
      const seedItems = {
        "Payroll & Benefits": [
          ...
          {
            description: "Benefits & insurance",
            amount: 24000,
            date: "2026-04-30",
          },
        ],
        "Facilities & Rent": [
          {
            description: "Building lease — April",
            amount: 18500,
            date: "2026-04-01",
          },
          {
        ...
        "Curriculum & Books": [
          {
            description: "Math textbooks — Grade 3",
            amount: 4200,
            date: "2026-03-22",
          },
          {
            description: "Reading workbooks",
            amount: 1850,
            date: "2026-04-05",
          },
        ],
        Insurance: [
          {
            description: "Liability premium — Q2",
            amount: 9500,
            date: "2026-04-01",
          },
        ],
        "Marketing & Admissions": [
          {
            description: "Open house event materials",
            amount: 850,
            date: "2026-04-12",
          },
          {
            description: "Online ads — April",
            amount: 1200,
            date: "2026-04-30",
          },
        ],
        "Tech & Equipment": [
          {
            description: "Chromebook replacements (4)",
            amount: 1600,
            date: "2026-04-08",
          },
          {
  ...
  if (view === "categories") {
    const pieData = categories.map((c) => ({
      name: c.name,
      value: c.spent,
      color: CATEGORY_COLORS[c.name] || C.blue,
    }));
    return (
      <div className="space-y-5">
        <SectionHead
          ...
          sub="Tap any category to see line items"
          action={
            <Btn
              kind="ghost"
            ...
            <PieChart>
              ...
              >
                {pieData.map((d, i) => (
                  <Cell
                    key={i}
                    fill={CATEGORY_COLORS[d.name] || C.blue}
            ...
            {categories.map((c) => {
              ...
              return (
                <button
                  ...
                  className="w-full p-3 rounded-xl flex items-center gap-3 hover:opacity-80 transition-opacity text-left"
                  style={{ background: "#F9FAFC" }}
                >
                  <span
                    className="w-3 h-3 rounded-full flex-shrink-0"
                    style={{ background: CATEGORY_COLORS[c.name] || C.blue }}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-sm" style={{ color: C.ink }}>
                      {c.name}
                    ...
                    <div className="text-xs mt-0.5" style={{ color: C.ink3 }}>
                      {fmtMoney(c.spent)} of {fmtMoney(c.budget)} · {catPct}%
                    </div>
                  </div>
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="💰"
        title="Budget"
        sub={`Annual: ${fmtMoneyShort(SCHOOL_BUDGET_TOTAL)} · Aug–May`}
      />

      <Card className="p-6" onClick={() => setView("categories")}>
        <div
          className="text-xs font-bold uppercase tracking-wider mb-1 text-center"
          style={{ color: C.ink2 }}
        >
          Consumption
        </div>
        ...
        >
          ...
          <div className="text-right">
            ...
            <div
              ...
              style={{ color: C.green }}
            >
              {fmtMoney(SCHOOL_BUDGET_TOTAL - totalSpent)}
            </div>
          </div>
        </div>
      </Card>
...
const OwnerScholarships = ({ stepUp }) => {
  ...
  const pendingByAge = {
    ...
    aging: pending.filter(
      (s) => daysSince(s.sent) >= 7 && daysSince(s.sent) < 14,
    ).length, // 7-13 days, amber
    stale: pending.filter(
      (s) => daysSince(s.sent) >= 14 && daysSince(s.sent) < 20,
    ).length, // 14-19, orange
  ...
  const turnaroundPct = turnaroundTimes.length
    ? Math.round((fastApproval / turnaroundTimes.length) * 100)
    : 100;

  const sortedPending = [...pending].sort(
    (a, b) => daysSince(b.sent) - daysSince(a.sent),
  );
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="🎓"
        title="Scholarships"
        sub={`${students} students · ${fmtMoney(total)} awarded YTD`}
      />
      ...
      >
        <div className="flex items-start justify-between gap-2 mb-4">
          <div>
            <h3 className="text-lg font-bold" style={{ color: C.ink }}>
              Step Up Payment Approvals
            </h3>
            <p className="text-sm mt-0.5" style={{ color: C.ink3 }}>
              Parents must approve each payment in Step Up portal · Director
              chases weekly
            </p>
          </div>
          <Pill tone={pendingByAge.redflag > 0 ? "red" : "amber"}>
        ...
        >
          <div>
            ...
            <div className="text-sm mt-0.5" style={{ color: C.ink2 }}>
              Avg {avgTurnaround} days to approval · {turnaroundPct}% closed
              within 14 days
            </div>
          </div>
          <Pill
        ...
        <div className="space-y-2">
          ...
          {sortedPending.length === 0 && (
            <div className="py-6 text-center text-sm" style={{ color: C.ink3 }}>
              ✓ All approvals cleared. Great work.
            </div>
          )}
        </div>
      </Card>
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="👥"
        title="Staff"
        sub={`${staff.length} total · ${counts.present} present today`}
      />
...
const OwnerWaitlist = ({ waitlist, classrooms }) => {
  const preschoolCount = waitlist.filter((w) =>
    PRESCHOOL_PROGRAMS.includes(w.program),
  ).length;
  const k8Count = waitlist.filter((w) =>
    K8_PROGRAMS.includes(w.program),
  ).length;

  const preschoolOpen = classrooms
    .filter((c) => PRESCHOOL_PROGRAMS.includes(c.program))
    .reduce((a, c) => a + (c.capacity - c.enrolled), 0);
  const k8Open = classrooms
    .filter((c) => K8_PROGRAMS.includes(c.program))
    .reduce((a, c) => a + (c.capacity - c.enrolled), 0);
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="📝"
        title="Waitlist"
        sub="High-level view · two categories"
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card accent={C.purple} className="p-6">
          ...
          >
            ...
            <div
              className="text-2xl font-extrabold"
              style={{ color: preschoolOpen > 0 ? C.green : C.red }}
            >
              {preschoolOpen}
            </div>
          </div>
        ...
        <Card accent={C.blue} className="p-6">
          ...
          >
            ...
            <div
              className="text-2xl font-extrabold"
              style={{ color: k8Open > 0 ? C.green : C.red }}
            >
              {k8Open}
            </div>
          </div>
      ...
      <Card className="p-5">
        ...
        <div className="flex justify-center gap-4 mt-2 text-xs">
          <span className="flex items-center gap-1" style={{ color: C.ink2 }}>
            <span
              className="w-2 h-2 rounded-full"
  ...
  const submit = () => {
    ...
    if (activeType === "expense") {
      ...
      const newM = {
        ...
        issue: form.issue.slice(0, 100),
        priority: form.priority,
        status: "open",
        logged: today,
        estCost: parseInt(form.estCost) || 0,
      };
      ...
      const newW = {
        id: Date.now(),
        child: form.child.slice(0, 40),
        program: form.program,
        parent: form.parent.slice(0, 40),
        phone: form.phone || "",
        email: form.email || "",
        dateAdded: today,
        status: "inquiry",
      ...
      setLog([
        {
          ...
          type: "incident",
          date: today,
          student: form.student.slice(0, 40),
      ...
      const newSub = {
        id: Date.now(),
        date: today,
        coveringFor: form.coveringFor,
        subName: form.subName.slice(0, 40),
        calledBy: "Director",
      };
      ...
      return (
        <div className="space-y-3">
          {select(
            "staff",
            "Staff Member",
            initialStaff.map((s) => s.name),
          )}
          {select("dayType", "Day Type", [
            "sick",
            "personal",
            "vacation",
            "jury",
      ...
      return (
        <div className="space-y-3">
          {select(
            ...
            "Covering For",
            staff
              ? staff.filter((s) => s.role === "Teacher").map((s) => s.name)
              : [],
          )}
    ...
    <div className="space-y-5">
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 flex gap-2 justify-end">
          ...
          <Btn kind="primary" onClick={submit} icon={Plus}>
            Log Entry
          </Btn>
        </div>
      </Card>
      ...
      <Card className="p-5">
        ...
        <div className="space-y-2">
          {combinedFeed.map((item) => {
            const typeMeta =
              LOG_TYPES.find((t) => t.id === item.type) || LOG_TYPES[0];
                  ...
                  >
                    {item.type === "expense" && (
                      <>
                        ...
                        <span style={{ color: C.ink3, fontWeight: 400 }}>
                          · {item.description}
                        </span>
                      </>
                    )}
                    {item.type === "pto" && (
                      <>
                        {item.staff} · {item.dayType} ({item.days}d)
                      </>
                    )}
                    {item.type === "incident" && (
                      <>
                        {item.student} · {item.severity} · {item.area}
                      </>
                    )}
                    {item.type === "removal" && (
                      <>
                    ...
                    {item.type === "waitlist_add" && (
                      <>
                        {item.child} · {item.program}
                      </>
                    )}
                    {item.type === "substitute" && (
                      <>
                        {item.subName} covering {item.coveringFor}
                      </>
                    )}
                  </div>
                  <div
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="💰"
        title="My Budget"
        sub={`$${DIRECTOR_BUDGET_TOTAL.toLocaleString()} · Aug through end of May`}
      />
        ...
        <div className="space-y-2">
          {groups.map((g) => (
            ...
            >
              <span
                className="w-3 h-3 rounded-full flex-shrink-0"
                style={{ background: CATEGORY_COLORS[g.name] }}
              />
              <div className="flex-1 min-w-0">
                <div className="font-bold text-sm" style={{ color: C.ink }}>
                  {g.name}
              ...
              <div className="font-extrabold text-sm" style={{ color: C.ink }}>
                {fmtMoney(g.total)}
              </div>
            </div>
          ))}
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="📝"
        title="Waitlist"
        sub={`${waitlist.length} families · use the Log tab to add new ones`}
      />
      ...
      <Card className="p-5">
        <div className="space-y-2">
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs mt-0.5" style={{ color: C.ink3 }}>
                    Joined {w.dateAdded} · {daysSince(w.dateAdded)}d waiting
                  </div>
                </div>
              </div>
          ...
          {list.length === 0 && (
            <div className="py-8 text-center text-sm" style={{ color: C.ink3 }}>
...
const DirectorPayroll = ({
  ...
  const teacherList = staff.filter(
    (s) => s.role === "Teacher" || s.role === "Front Office",
  );
  ...
  const balanceTone = (n) =>
    n <= 0 ? "redSoft" : n <= 2 ? "amberSoft" : "greenSoft";
  const balanceColor = (n) => (n <= 0 ? C.red : n <= 2 ? C.amber : C.green);

  // CRUD helpers (each section)
  const addCC = () =>
    setChildCare([...childCare, { id: Date.now(), staffId: "", amount: "" }]);
  const updCC = (i, f, v) =>
    setChildCare(childCare.map((r, idx) => (idx === i ? { ...r, [f]: v } : r)));
  const rmCC = (i) => setChildCare(childCare.filter((_, idx) => idx !== i));

  const addOD = () =>
    setOtherDed([...otherDed, { id: Date.now(), loanId: "", amount: "" }]);
  const updOD = (i, f, v) =>
    setOtherDed(otherDed.map((r, idx) => (idx === i ? { ...r, [f]: v } : r)));
  const rmOD = (i) => setOtherDed(otherDed.filter((_, idx) => idx !== i));

  const addPTO = () =>
    setPto([
      ...pto,
      { id: Date.now(), staffId: "", startDate: "", endDate: "" },
    ]);
  const updPTO = (i, f, v) =>
    setPto(pto.map((r, idx) => (idx === i ? { ...r, [f]: v } : r)));
  const rmPTO = (i) => setPto(pto.filter((_, idx) => idx !== i));

  const addBD = () =>
    setBirthday([...birthday, { id: Date.now(), staffId: "", date: "" }]);
  const updBD = (i, f, v) =>
    setBirthday(birthday.map((r, idx) => (idx === i ? { ...r, [f]: v } : r)));
  const rmBD = (i) => setBirthday(birthday.filter((_, idx) => idx !== i));

  const addHTA = () =>
    setHoursToAdd([
      ...hoursToAdd,
      { id: Date.now(), staffId: "", hours: "", type: "After-care" },
    ]);
  const updHTA = (i, f, v) =>
    setHoursToAdd(
      hoursToAdd.map((r, idx) => (idx === i ? { ...r, [f]: v } : r)),
    );
  ...
  const handleSubmit = () => {
    const payload = {
      ...
      pto: pto.map((r) => ({
        ...
        balanceAfter:
          ptoBalance(Number(r.staffId)) - daysBetween(r.startDate, r.endDate),
      })),
      ...
      holidayExceptions: periodHolidays.map((h) => ({
        date: h.date,
        name: h.name,
        excluded: (holidayExceptions[h.id] || []).map(
          (sid) => staff.find((s) => s.id === sid)?.name || "Unknown",
        ),
      })),
      notes,
    };
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="💵"
        title="Payroll Ready"
        sub={`Pay period ending ${periodEnd} · Replaces your bi-weekly email to Owner`}
      />

      {/* Payroll countdown banner */}
      <Card
        className="p-4 flex items-center gap-3"
        accent={payrollDays <= 3 ? C.red : payrollDays <= 7 ? C.amber : C.blue}
      >
        <div
          className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
          style={{ background: C.blueSoft }}
        ...
        <Pill
          tone={payrollDays <= 3 ? "red" : payrollDays <= 7 ? "amber" : "blue"}
        >
          {payrollDays}d
        </Pill>
      </Card>
      ...
      <Card className="p-5">
        ...
        <p className="text-xs mb-3" style={{ color: C.ink3 }}>
          Tuition-style deductions taken from staff paychecks.
        </p>
        <div className="space-y-2">
          {childCare.length === 0 && (
            <div
          ...
          {childCare.map((row, i) => (
            <div
              ...
              style={{ gridTemplateColumns: "1fr 110px 28px" }}
            >
              {staffSelect(row.staffId, (v) => updCC(i, "staffId", v))}
              <input
                type="number"
                value={row.amount}
                onChange={(e) => updCC(i, "amount", e.target.value)}
                placeholder="$0.00"
                className="px-3 py-2 rounded-lg text-sm"
                style={{ background: "#fff", border: `1px solid ${C.line}` }}
              />
              <button onClick={() => rmCC(i)} style={{ color: C.ink3 }}>
                ×
              </button>
            </div>
          ))}
      ...
      <Card className="p-5">
        ...
        <p className="text-xs mb-3" style={{ color: C.ink3 }}>
          Loans, advances. Remaining balance auto-tracks across pay periods.
        </p>
        <div className="space-y-2">
          {otherDed.length === 0 && (
            <div
          ...
          {otherDed.map((row, i) => {
            const loan = otherDeductions.find(
              (l) => l.id === Number(row.loanId),
            );
            ...
            return (
              <div
                key={row.id}
                className="grid gap-2 items-center"
                style={{ gridTemplateColumns: "1fr 90px 140px 28px" }}
              >
                <select
                  value={row.loanId}
                  onChange={(e) => updOD(i, "loanId", e.target.value)}
                  className="px-3 py-2 rounded-lg text-sm"
                  style={{
                    background: "#fff",
                    border: `1px solid ${C.line}`,
                ...
                <input
                  type="number"
                  value={row.amount}
                  onChange={(e) => updOD(i, "amount", e.target.value)}
                  placeholder="$0.00"
                  className="px-3 py-2 rounded-lg text-sm"
                  style={{ background: "#fff", border: `1px solid ${C.line}` }}
                />
                <div
                  className="px-2 py-2 rounded-lg text-xs text-center font-bold"
                  style={{
                    background: loan
                      ? balanceTone(newBal) === "redSoft"
                        ? C.redSoft
                        : balanceTone(newBal) === "amberSoft"
                          ? C.amberSoft
                          : C.greenSoft
                      : "#F1F5F9",
                    color: loan ? balanceColor(newBal) : C.ink3,
                  }}
                >
                  {loan ? `$${newBal.toLocaleString()} left` : "—"}
                </div>
                <button onClick={() => rmOD(i)} style={{ color: C.ink3 }}>
                  ×
                </button>
              </div>
            );
      ...
      <Card className="p-5">
        ...
        <p className="text-xs mb-3" style={{ color: C.ink3 }}>
          Single day or date range — system auto-counts and deducts from
          balance.
        </p>
        <div className="space-y-2">
          {pto.length === 0 && (
          ...
          {pto.map((row, i) => {
            const balance = row.staffId
              ? ptoBalance(Number(row.staffId))
              : null;
            ...
            return (
              <div
                key={row.id}
                className="grid gap-2 items-center"
                style={{ gridTemplateColumns: "1fr 1.3fr 140px 28px" }}
              >
                ...
                <div className="flex gap-1 items-center">
                  <input
                    type="date"
                    value={row.startDate}
                    onChange={(e) => updPTO(i, "startDate", e.target.value)}
                  ...
                  <input
                    type="date"
                    value={row.endDate}
                    onChange={(e) => updPTO(i, "endDate", e.target.value)}
                    className="flex-1 min-w-0 px-2 py-2 rounded-lg text-xs"
                    style={{
                      background: "#fff",
                      border: `1px solid ${C.line}`,
                  ...
                  {row.startDate && (
                    <span
                      ...
                      style={{ background: "#EDE9FE", color: C.purple }}
                    >
                      {days}d
                    </span>
                  )}
                ...
                <div
                  ...
                  style={{
                    ...
                    color:
                      afterBalance === null
                        ? C.ink3
                        : balanceColor(afterBalance),
                  }}
                >
                  {afterBalance === null
                    ? "—"
                    : `${afterBalance} left ${afterBalance <= 0 ? "⚠" : ""}`}
                </div>
                <button onClick={() => rmPTO(i)} style={{ color: C.ink3 }}>
                  ×
                </button>
              </div>
            );
      ...
      <Card className="p-5" accent={C.blue}>
        ...
        <p className="text-xs mb-3" style={{ color: C.ink3 }}>
          Separate from PTO · one per staff per year · auto-blocks duplicates.
        </p>
        <div className="space-y-2">
          {birthday.length === 0 && (
            <div
              ...
              <div key={row.id}>
                ...
                >
                  ...
                  <input
                    type="date"
                    value={row.date}
                    onChange={(e) => updBD(i, "date", e.target.value)}
                  ...
                  />
                  <button onClick={() => rmBD(i)} style={{ color: C.ink3 }}>
                    ×
                  </button>
                </div>
                {alreadyUsed && (
                  <div className="text-xs mt-1 ml-1" style={{ color: C.red }}>
                    ⚠ {s.name} already used birthday this year
                  </div>
                )}
              </div>
            );
      ...
      <Card className="p-5" accent={C.red}>
        <div className="flex items-baseline justify-between mb-2">
          ...
          >
            5 · Hours to Add in ADP{" "}
            <span
              className="ml-1 inline-block px-1.5 py-0.5 rounded text-[9px]"
        ...
        <p className="text-xs mb-3" style={{ color: C.ink3 }}>
          Hours not captured by ADP time clock — Owner adds these manually.
          Total hours for the period, no dates needed.
        </p>
        <div className="space-y-2">
          {hoursToAdd.length === 0 && (
            <div
          ...
          {hoursToAdd.map((row, i) => (
            <div
              key={row.id}
              className="grid gap-2 items-center"
              style={{ gridTemplateColumns: "1fr 90px 130px 28px" }}
            >
              ...
              <input
                ...
                step="0.25"
                value={row.hours}
                onChange={(e) => updHTA(i, "hours", e.target.value)}
                placeholder="0 hrs"
                className="px-3 py-2 rounded-lg text-sm"
                style={{ background: "#fff", border: `1px solid ${C.line}` }}
              />
              <select
                value={row.type}
                onChange={(e) => updHTA(i, "type", e.target.value)}
              ...
              <button onClick={() => rmHTA(i)} style={{ color: C.ink3 }}>
                ×
              </button>
            </div>
          ))}
      ...
      {periodHolidays.length > 0 && (
        <Card
          className="p-5"
          style={{ border: `2px solid ${C.amber}`, background: "#FFFBF0" }}
        >
          <h3
            className="text-xs font-bold uppercase tracking-widest mb-3"
            style={{ color: C.ink3 }}
          >
            6 · Holiday This Period
          </h3>
              ...
              >
                ...
                >
                  ...
                  <span
                    ...
                    style={{ color: C.green }}
                  >
                    ALL {teacherList.length} staff receive this holiday by
                    default
                  </span>
                </div>
                <div
                  className="p-3 rounded-lg mb-3 text-xs font-medium"
                  style={{ background: C.amberSoft, color: C.amber }}
                >
                  ⚠ <b>Only add staff who do NOT qualify.</b> Pick from the
                  dropdown. Leave empty if everyone qualifies.
                </div>
                <div
                ...
                >
                  <div
                    ...
                    style={{ color: C.ink3 }}
                  >
                    Staff excluded from this holiday
                  </div>
                  {excluded.length > 0 && (
                      ...
                      {excluded.map((sid) => {
                        ...
                        return (
                          ...
                          >
                            {s?.name}
                            <button
                              onClick={() => toggleExclusion(h.id, sid)}
                              style={{ color: C.red, opacity: 0.7 }}
                            >
                              ×
                            </button>
                          </span>
                  ...
                  <select
                    onChange={(e) => {
                      if (e.target.value) {
                        toggleExclusion(h.id, Number(e.target.value));
                        e.target.value = "";
                      }
                    }}
                    className="w-full px-3 py-2 rounded-lg text-sm"
      ...
      <Card className="p-5">
        ...
        <p className="text-xs mb-3" style={{ color: C.ink3 }}>
          For anything that didn't fit a structured row. Short and specific.
        </p>
        <div className="space-y-3">
          <div>
            <div className="flex justify-between mb-1">
              <label className="text-xs font-bold" style={{ color: C.ink3 }}>
                Preschool Notes
              </label>
              <span className="text-[10px]" style={{ color: C.ink3 }}>
                {notes.preschool.length} / 200
              </span>
            </div>
            <textarea
              value={notes.preschool}
              maxLength={200}
              onChange={(e) =>
                setNotes({ ...notes, preschool: e.target.value })
              }
          ...
          <div>
            <div className="flex justify-between mb-1">
              ...
              <span className="text-[10px]" style={{ color: C.ink3 }}>
                {notes.elementary.length} / 200
              </span>
            </div>
            <textarea
              value={notes.elementary}
              maxLength={200}
              onChange={(e) =>
                setNotes({ ...notes, elementary: e.target.value })
              }
              placeholder="Optional"
              className="w-full px-3 py-2 rounded-lg text-sm"
              style={{
      ...
      <Card className="p-5">
        <div className="grid grid-cols-3 gap-2 mb-3">
          ...
          <button
            ...
            className="col-span-2 px-4 py-3 rounded-xl text-sm font-bold"
            style={{ background: C.blue, color: "#fff" }}
          >
            Mark Payroll Ready →
          </button>
        </div>
        <p className="text-xs text-center" style={{ color: C.ink3 }}>
          Submit sends Owner an email summary <b>and</b> triggers a BIG alert on
          Owner's dashboard.
        </p>
      </Card>
          ...
          <div className="space-y-2">
            {payrollHistory.map((h) => (
              ...
              >
                <div>
                  <div className="font-bold text-sm" style={{ color: C.ink }}>
                    Pay period ending {h.periodEnding}
                  </div>
                  <div className="text-xs" style={{ color: C.ink3 }}>
                    {h.childCare.length} child-care · {h.pto.length} PTO ·{" "}
                    {h.hoursToAdd.length} hour adjustments
                  </div>
                </div>
...
const OwnerCashFlow = ({ data, years, insights }) => {
  ...
  const filteredInsights =
    insightFilter === "all"
      ? insights
      : insights.filter((i) => i.tone === insightFilter);
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        icon="📊"
        title="Cash Flow — Year over Year"
        sub="Where the money goes, compared across years · AI-categorized from QuickBooks"
      />
      ...
      <Card className="p-4 flex flex-col md:flex-row items-start md:items-center gap-3">
        <div className="flex-1">
          ...
          <div className="text-sm mt-1" style={{ color: C.ink }}>
            Last upload: April 2026 statement ·{" "}
            <span style={{ color: C.green, fontWeight: 600 }}>
              ✓ AI-categorized
            </span>
          </div>
      ...
      <Card className="p-5" accent={C.purple}>
        <div className="flex items-baseline justify-between mb-3 flex-wrap gap-2">
          <div>
            ...
            <div className="text-xs mt-1" style={{ color: C.ink3 }}>
              Auto-generated from YoY trends
            </div>
          </div>
            ...
            const bg =
              i.tone === "red"
                ...
                : i.tone === "amber"
                  ...
                  : C.greenSoft;
            const fg =
              i.tone === "red" ? C.red : i.tone === "amber" ? C.amber : C.green;
            return (
              <div
                key={idx}
      ...
      <Card className="p-5">
        <div
          ...
          style={{ color: C.ink2 }}
        >
          YoY Spending — {selected.length}{" "}
          {selected.length === 1 ? "Category" : "Categories"}
        </div>
        {selected.length === 0 ? (
          <div className="text-center text-sm py-8" style={{ color: C.ink3 }}>
            Pick a category above to see the chart.
          </div>
        ) : (
        ...
        <div className="text-[10px] text-center mt-2" style={{ color: C.ink3 }}>
          2026 is YTD (Jan–Apr) — partial year
        </div>
      </Card>
      ...
      <Card className="p-5" accent={C.blue}>
        <div
          ...
          style={{ color: C.ink2 }}
        >
          Combined Total — Selected Categories
        </div>
        <ResponsiveContainer width="100%" height={200}>
          <LineChart
            data={totalByYear}
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      ...
                      style={{
                        color:
                          change > 5 ? C.red : change < -3 ? C.green : C.ink3,
                      }}
                    >
                      {change !== null
          ...
          <>
            {tab === "overview" && (
              <OwnerOverview
                ...
                dismissPayrollAlert={dismissPayrollAlert}
                setTab={setTab}
              />
            )}
            {tab === "enrollment" && (
            ...
            {tab === "classrooms" && (
              <OwnerClassrooms classrooms={classrooms} />
            )}
            {tab === "cashflow" && (
              <OwnerCashFlow
                data={CASHFLOW_DATA}
            ...
            {tab === "compliance" && (
              <OwnerCompliance compliance={compliance} />
            )}
            {tab === "tasks" && <OwnerTasks tasks={tasks} />}
            {tab === "maintenance" && (
              <OwnerMaintenance maintenance={maintenance} />
            )}
            ...
            {tab === "waitlist" && (
              <OwnerWaitlist waitlist={waitlist} classrooms={classrooms} />
            )}
          </>
        )}
        {role === "director" && (
          <>
            {tab === "log" && (
              <DirectorLog
                ...
                staff={staff}
                payrollDays={daysUntil(NEXT_PAYROLL)}
              />
            )}
            {tab === "payroll" && (
              <DirectorPayroll
            ...
            {tab === "waitlist" && (
              <DirectorWaitlist waitlist={waitlist} setWaitlist={setWaitlist} />
            )}
          </>
        )}
      </main>
